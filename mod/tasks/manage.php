<?php
	...
	// Get the current page's owner
		...
		$area2 .= elgg_view_title(elgg_echo('tasks:tasksmanageone'), false);
		
	// If we've been given a task to edit, grab it
		if ($this_guid = get_input('task',0)) {
			$entity = get_entity($this_guid);
			if ($entity->canEdit()) {
				$area2 .= elgg_view('tasks/form_manage',array('entity' => $entity, 'container_guid' => $container));
			}
		} else {
			$area2 .= elgg_view('tasks/form_manage', array('container_guid' => $container));
		}
