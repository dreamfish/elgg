<?php
	...
	$forward_url = $dgroup->getURL();
	
	
?>
<div class="contentWrapper">
<form action="<?php echo $vars['url']; ?>action/dgroups/invite" method="post">
	...
	if ($friends = get_entities_from_relationship('friend',$_SESSION['guid'],false,'user','',0,'',9999)) {
		...
		// echo elgg_view('sharing/invite',array('shares' => $shares, 'owner' => $owner, 'dgroup' => $dgroup));
	
	?>
	<input type="hidden" name="forward_url" value="<?php echo $forward_url; ?>" />
	<input type="hidden" name="dgroup_guid" value="<?php echo $dgroup->guid; ?>" />
